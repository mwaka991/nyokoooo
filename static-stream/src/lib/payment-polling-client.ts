...
export function createPaymentPoller(options: PollingOptions) {
  const config = {
    maxAttempts: options.maxAttempts || 30,  // Increased from 20 to 30 for longer wait time
    initialDelayMs: options.initialDelayMs || 3000,  // Increased from 1000 to 3000 (3 seconds)
    maxDelayMs: options.maxDelayMs || 8000,
    backoffMultiplier: options.backoffMultiplier || 1.5,
    jitterFactor: options.jitterFactor || 0.2,
    timeoutSeconds: options.timeoutSeconds || 120,  // Increased from 60 to 120 seconds
  };
